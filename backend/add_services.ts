...
async function main() {
  ...
  try {
    ...
    for (const s of MOCK_SERVICES) {
      ...
      let service;
      if (existing) {
        console.log(`  ⏭️ Service already exists: ${s.name}`);
        service = existing;
      } else {
    ...
    console.log(`✅ Processed ${createdServices.length} services with pricing`);
    ...
    console.error('❌ Service population failed:', error);
    throw error;
  }
}
